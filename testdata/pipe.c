...
t_testdata ls_pipe_grep(void)
{
  ...
  t_and_or *last_andor = NULL;
	...
	last_cmd->u.simple.token_list = gen_token_list(1,
			TOKEN_UNQUOTED_WORD, "ls");
  last_andor = get_last_and_or(&ast_ptr);
  last_cmd = lstget_command(lstadd_back_command(&last_andor->pipeline->command_list, CMD_SIMPLE));
