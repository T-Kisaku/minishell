...
static size_t list_size(t_list *lst)
{
    size_t n = 0;
    while (lst)
    {
...
static void assert_token_list_equal(t_list *exp, t_list *act, const char *path)
{
    size_t idx = 0;
    t_list *e = exp;
    t_list *a = act;
    while (e && a)
    {
        t_token_content *et = e->content;
        t_token_content *at = a->content;
        if (et->type != at->type || strcmp(et->value, at->value) != 0)
        {
            fprintf(stderr, "%s[%zu] token mismatch: exp(%d,'%s') act(%d,'%s')\n",
                    path, idx, et->type, et->value, at->type, at->value);
            assert(0);
        }
        idx++;
        e = e->next;
    ...
    if (e || a)
    {
        fprintf(stderr, "%s size mismatch: exp=%zu act=%zu\n",
                path, list_size(exp), list_size(act));
        assert(0);
    }
}
                                      ...
                                      const char *path)
{
    ...
    const char *e_name = exp.filename ? exp.filename
                                       : (exp.filename_token ? exp.filename_token->value : NULL);
    const char *a_name = act.filename ? act.filename
                                       : (act.filename_token ? act.filename_token->value : NULL);
    if ((e_name || a_name) && (!e_name || !a_name || strcmp(e_name, a_name) != 0))
    {
...
static void assert_command_equal(t_command *exp, t_command *act, const char *path);

static void assert_pipeline_equal(t_pipeline *exp, t_pipeline *act, const char *path)
{
    size_t idx = 0;
...
static void assert_and_or_equal(t_and_or *exp, t_and_or *act, const char *path)
{
