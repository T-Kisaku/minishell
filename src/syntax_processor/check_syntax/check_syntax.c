...
static t_error *check_head(t_list **cur, t_list **prev) {
  ...
  error = set_token_group(&cur_group, ((t_token *)(*cur)->content)->type);
  if (error)
    return (error);
...
static t_error *check_body(t_list **cur, t_list **prev) {
  ...
  error = set_token_group(&prev_group, ((t_token *)(*prev)->content)->type);
  if (error)
    return (error);
  while (*cur) {
    error = set_token_group(&cur_group, ((t_token *)(*cur)->content)->type);
...
static t_error *check_tail(t_list **list, t_minishell_state *shell) {
  ...
  if (group == TOKEN_GROUP_CONTROL_OP) {
    ...
    return (new_error(EXIT_INTERNAL_ERR, "syntax error: token group not set"));
}
