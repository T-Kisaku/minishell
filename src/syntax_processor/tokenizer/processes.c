...
int	create_token(t_token_context *tc)
{
	...
	new_token->content->value = NULL;
	new_token->content->type = TOKEN_UNQUOTED_WORD;
	if (*(tc->head) == NULL)
		*(tc->head) = new_token;
	else
...
int	set_token(t_token_context *tc, e_token_type type)
{
	tc->cur_token->content->type = type;
	tc->cur_token->content->value = malloc(sizeof(char) *
											(tc->cur_str - tc->start_str + 1));
	if (!tc->cur_token->content->value)
		return (write_error("malloc error \"value\""));
	ft_memcpy(tc->cur_token->content->value, tc->start_str, tc->cur_str
			- tc->start_str);
	tc->cur_token->content->value[tc->cur_str - tc->start_str] = '\0';
