...
int	set_token(t_token_context *ctx, e_token_type type)
{
	...
	cur_token_content->value = malloc(sizeof(char) *
									(ctx->cur_str - ctx->start_str + 1));
	if (!cur_token_content->value)
		return (write_error("malloc error \"value\""));
	ft_memcpy(cur_token_content->value, ctx->start_str, ctx->cur_str
			- ctx->start_str);
